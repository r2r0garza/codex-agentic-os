...
## Resume Notes
The deterministic repository index now has its language-neutral contract plus Git-backed tracked-file discovery and SHA-256 worktree hashing with explicit filtering. Next run should choose exactly one remaining unchecked task; Plan 0002's Python AST extraction item is the natural continuation. The full local pytest suite passes with 18 tests. Follow the README credential policy: create `.env.example` only when configuration is needed, keep `.env` untracked, and explicitly tell the user what to populate and which live checks could not run without keys.
