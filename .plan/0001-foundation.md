...
## Resume Notes
The deterministic repository index now has its language-neutral contract, Git-backed tracked-file discovery and hashing, Python AST extraction, and deterministic clean artifact builds. Next run should choose exactly one remaining unchecked task; Plan 0002's incremental rebuild and clean-output equivalence item is the natural continuation. The full local pytest suite passes with 22 tests. Follow the README credential policy: create `.env.example` only when configuration is needed, keep `.env` untracked, and explicitly tell the user what to populate and which live checks could not run without keys.
