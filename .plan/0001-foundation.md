...
## Resume Notes
The deterministic repository index now has its language-neutral contract, Git-backed tracked-file discovery and hashing, Python AST extraction, byte-equivalent clean and incremental artifact builds, CLI commands, and optional repository-managed pre-commit refresh. Next run should choose exactly one remaining unchecked task; Plan 0002's CI clean-rebuild drift verification is the natural continuation. Follow the README credential policy: create `.env.example` only when configuration is needed, keep `.env` untracked, and explicitly tell the user what to populate and which live checks could not run without keys.
