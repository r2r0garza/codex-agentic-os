...
## Resume Notes
The deterministic repository index now has its language-neutral contract, Git-backed tracked-file discovery and hashing, and Python AST extraction. Next run should choose exactly one remaining unchecked task; Plan 0002's deterministic manifest/JSONL serialization, atomic writes, and clean rebuild item is the natural continuation. The full local pytest suite passes with 20 tests. Follow the README credential policy: create `.env.example` only when configuration is needed, keep `.env` untracked, and explicitly tell the user what to populate and which live checks could not run without keys.
